...
public class AwardServiceImpl implements AwardService {
	...
	@Override
	@Transactional
	public AwardExecution addAward(Award award, MultipartFile thumbnail) {
	...
	@Override
	@Transactional
	public AwardExecution modifyAward(Award award, MultipartFile thumbnail) {
		if (award != null && award.getShopId() != null) {
			award.setLastEditTime(new Date());
			if (thumbnail != null) {
				addThumbnail(award, thumbnail);
	...
	private void addThumbnail(Award award,MultipartFile thumbnail) {
		if (thumbnail != null) {
			Result<String> awardResult = new UploadController().upload(thumbnail);
			if (awardResult.isSuccess()) {
				award.setAwardImg((String) awardResult.getData());
			} else {
				throw new RuntimeException("addAwardImg error: " + awardResult.getErrorMsg());
			}
		}
	}
